...
from selenium_wrapper_3.addons.kor_date_parser import convert_date
from selenium_wrapper_3.builder import ChromeBuilder
from selenium_wrapper_3.driver import Driver
from selenium_wrapper_3.node import *  # type: ignore[attr-defined]
from selenium_wrapper_3.util import *  # type: ignore[attr-defined]
...
for subject in enable_subjects:
    ...
    with frame(IFrame(id="fulliframe")):
        ...
        courses = subject / __ // Div(class_="xnsti-left")
    ...
    for title in titles:
        ...
        with frame(IFrame(id="tool_content")):
            total_dur = text(Span(text="재생 시간") / __ / Span(1))

            cur_dur = text(Span(text="학습 진행 상태") / __ / Span(1)).split("(")[0]
